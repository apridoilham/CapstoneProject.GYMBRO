...
const profileSimpleSchema = z.object({
  ...
  heightCm: z.preprocess(
    ...
    z.number({invalid_type_error: "Height must be a number."}).positive().min(50).max(300).optional().nullable()
  ),
  currentWeightKg: z.preprocess(
    (val) => (val === '' || val === null || val === undefined ? undefined : (typeof val === 'string' ? parseFloat(val) : val)),
    z.number({invalid_type_error: "Weight must be a number."}).positive().min(20).max(500).optional().nullable()
  ),
...
const SocialMediaLink = ({ href, icon: Icon, platformName }: { href?: string; icon: React.ElementType; platformName: string }) => {
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-primary transition-colors" aria-label={platformName}>
      <Icon size={22} />
    </a>
  );
};
...
export default function VerySimpleProfilePage() {
  ...
  const handleProfileUpdate: SubmitHandler<ProfileSimpleFormValues> = async (data) => {
    console.log("Updating profile with (simple):", data);
    ...
    setUser(updatedUser);
    localStorage.setItem('gymBroUserProfile', JSON.stringify(updatedUser)); // Simpan ke localStorage
            ...
            <CardHeader className="p-6 border-b border-zinc-700/80">
              <div className="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="flex-1 text-center sm:text-left pt-2">
                  ...
                  <p className="text-gray-400 text-xs mt-2">Age: {user.age} years</p>
                </div>
                <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-md w-[90vw] bg-zinc-900 border-zinc-700 text-white rounded-lg">
                    <DialogHeader>
                      ...
                      <DialogDescription className="text-gray-400">Update your stats.</DialogDescription>
                    </DialogHeader>
                      ...
                      <FormFieldComponent label="Date of Birth" id="s-dob" error={errors.dateOfBirth?.message}>
                       ...
                       <div className="grid grid-cols-2 gap-4">
                        <FormFieldComponent label="Height (cm)" id="s-height" error={errors.heightCm?.message}>
                          <Input id="s-height" type="number" {...register("heightCm")} className="bg-zinc-800 border-zinc-700" />
                        </FormFieldComponent>
                        <FormFieldComponent label="Weight (kg)" id="s-weight" error={errors.currentWeightKg?.message}>
                          <Input id="s-weight" type="number" step="0.1" {...register("currentWeightKg")} className="bg-zinc-800 border-zinc-700" />
                        </FormFieldComponent>
                      </div>
            ...
            <CardContent className="p-6 space-y-5">
              <section>
                ...
                <div className="space-y-1.5">
                  ...
                  <InfoRow icon={CalendarDays} label="Born" value={new Date(user.dateOfBirth).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} />
                  <InfoRow icon={User} label="Age" value={user.age} unit="years" />
                ...
                <>
                  ...
                  <section>
                    ...
                    <div className="flex items-center space-x-5">
                      ...
                      <SocialMediaLink href={user.socialMedia.instagram ? `https://instagram.com/${user.socialMedia.instagram}` : undefined} icon={Instagram} platformName="Instagram" />
                      <SocialMediaLink href={user.socialMedia.twitter ? `https://x.com/${user.socialMedia.twitter}` : undefined} icon={Twitter} platformName="Twitter/X" />
