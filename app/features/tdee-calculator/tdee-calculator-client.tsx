...
import { Flame, Dumbbell, TrendingDown, TrendingUp, Scale, User, Calendar, Activity, TargetIcon, Utensils, InfoIcon, Loader2, ArrowLeft, HomeIcon } from 'lucide-react'; // Tambahkan HomeIcon
...
type Goal = "maintenance" | "mildLoss" | "weightLoss" | "extremeLoss" | "mildGain" | "weightGain";
...
export default function TdeeCalculatorClient() {
  ...
  const handleUnitSystemChange = (value: UnitSystem) => {
    const newWeight = inputs.unitSystem === "metric" && value === "imperial"
      ? parseFloat((inputs.weight * 2.20462).toFixed(1))
      : inputs.unitSystem === "imperial" && value === "metric"
      ? parseFloat((inputs.weight / 2.20462).toFixed(1))
      : inputs.weight;

    const newHeight = inputs.unitSystem === "metric" && value === "imperial"
      ? parseFloat((inputs.height / 2.54).toFixed(1))
      : inputs.unitSystem === "imperial" && value === "metric"
      ? parseFloat((inputs.height * 2.54).toFixed(1))
      : inputs.height;

    const newTargetWeight = inputs.targetWeight && inputs.unitSystem === "metric" && value === "imperial"
      ? parseFloat((inputs.targetWeight * 2.20462).toFixed(1))
      : inputs.targetWeight && inputs.unitSystem === "imperial" && value === "metric"
      ? parseFloat((inputs.targetWeight / 2.20462).toFixed(1))
      : inputs.targetWeight;
  ...
  const calculateTdee = () => {
    ...
    const weightInKg = inputs.unitSystem === "imperial" ? inputs.weight / 2.20462 : inputs.weight;
    const heightInCm = inputs.unitSystem === "imperial" ? inputs.height * 2.54 : inputs.height;

    if (weightInKg <= 0 || heightInCm <= 0 || inputs.age <= 0) {
      toast({ variant: "destructive", title: "Invalid Input", description: "Weight, height, and age must be positive values, Bro." });
    ...
    if (inputs.goal !== "maintenance" && tdee) {
        if (inputs.goal.includes("Loss") && inputs.targetWeight && inputs.targetWeight < weightInKg && inputs.weeksToTarget && inputs.weeksToTarget > 0) {
        ...
        } else if (inputs.goal.includes("Gain") && inputs.targetWeight && inputs.targetWeight > weightInKg && inputs.weeksToTarget && inputs.weeksToTarget > 0) {
    ...
    if (goalCalories) {
        ...
        const fatMax = Math.round((goalCalories * 0.30) / 9);
        const carbsMin = Math.round((goalCalories - (proteinMax * 4) - (fatMax * 9)) / 4);
        const carbsMax = Math.round((goalCalories - (proteinMin * 4) - (fatMin * 9)) / 4);
        ...
        setResult({
          ...
          carbs: { min: carbsMin > 0 ? carbsMin : 0, max: carbsMax > 0 ? carbsMax : 0 },
        });
    } else {
        setResult({ bmr, tdee });
    }
        ...
        >
          ...
          <header className="text-center mb-10 md:mb-12">
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">
              Calorie & <span className="text-primary">TDEE</span> Planner
            </h1>
            <p className="text-gray-400 mt-4 text-md md:text-lg max-w-2xl mx-auto">
              Estimate your daily calorie needs (TDEE) and plan your intake for weight goals. This is an estimate, Bro!
            </p>
          </header>

          <Card className="bg-zinc-900/70 border-zinc-700/60 shadow-2xl backdrop-blur-sm rounded-xl">
            <CardHeader className="pb-5">
              <CardTitle className="text-2xl text-white flex items-center"><Dumbbell size={24} className="mr-3 text-primary"/>Your Stats</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6 pt-2">
              <div className="flex justify-end">
                ...
                >
                  <div className="flex items-center space-x-1">
                    <RadioGroupItem value="imperial" id="imperial-tdee" className="h-3.5 w-3.5 border-gray-500 data-[state=checked]:border-primary data-[state=checked]:text-primary" />
                    <Label htmlFor="imperial-tdee" className="text-xs text-gray-400 cursor-pointer">lb, ft</Label>
                  </div>
                  <div className="flex items-center space-x-1">
                    <RadioGroupItem value="metric" id="metric-tdee" className="h-3.5 w-3.5 border-gray-500 data-[state=checked]:border-primary data-[state=checked]:text-primary" />
                    <Label htmlFor="metric-tdee" className="text-xs text-gray-400 cursor-pointer">kg, cm</Label>
                  </div>
                </RadioGroup>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="weight" type="number" value={inputs.weight} onChange={(e) => handleInputChange('weight', parseFloat(e.target.value) || 0)} className="mt-1 bg-zinc-800 border-zinc-700" />
                </div>
                <div>
                  <Label htmlFor="height" className="text-sm font-medium text-gray-300">Height ({heightUnit})</Label>
                  <Input id="height" type="number" value={inputs.height} onChange={(e) => handleInputChange('height', parseFloat(e.target.value) || 0)} className="mt-1 bg-zinc-800 border-zinc-700" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="age" className="text-sm font-medium text-gray-300">Age (years)</Label>
                  <Input id="age" type="number" value={inputs.age} onChange={(e) => handleInputChange('age', parseInt(e.target.value) || 0)} className="mt-1 bg-zinc-800 border-zinc-700" />
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-300 mb-1.5 block">Select your gender</Label>
                  <RadioGroup value={inputs.gender} onValueChange={(value) => handleInputChange('gender', value as Gender)} className="flex space-x-4">
              ...
              <div>
                <Label htmlFor="activityLevel" className="text-sm font-medium text-gray-300">Activity Level (Optional for BMR)</Label>
                <Select value={inputs.activityLevel} onValueChange={(value) => handleInputChange('activityLevel', value as ActivityLevel)}>
                  <SelectTrigger className="w-full mt-1 bg-zinc-800 border-zinc-700">
                    <SelectValue placeholder="Select activity level" />
                  </SelectTrigger>
                  <SelectContent className="bg-zinc-800 border-zinc-700 text-white">
                    <SelectItem value="sedentary">Sedentary (office job, minimal physical activity)</SelectItem>
                    <SelectItem value="light">Light activity (training 1-2 times per week)</SelectItem>
                    <SelectItem value="moderate">Moderate activity (training 3-5 times per week)</SelectItem>
                    <SelectItem value="active">Active (training 6-7 times per week)</SelectItem>
                    <SelectItem value="veryActive">Very Active (intense training daily, or physical job)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                ...
                <Label htmlFor="goal" className="text-sm font-medium text-gray-300">Your Goal</Label>
                 <Select value={inputs.goal} onValueChange={(value) => handleInputChange('goal', value as Goal)}>
                  ...
                  <SelectContent className="bg-zinc-800 border-zinc-700 text-white">
                    ...
                    <SelectItem value="extremeLoss">Extreme Weight Loss (~0.75 {weightUnit}/week) - Use with caution</SelectItem>
              ...
              {(inputs.goal.includes("Loss") || inputs.goal.includes("Gain")) && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <Input id="targetWeight" type="number" value={inputs.targetWeight || ''} onChange={(e) => handleInputChange('targetWeight', parseFloat(e.target.value) || undefined)} className="mt-1 bg-zinc-800 border-zinc-700" />
                    </div>
                    <div>
                        <Label htmlFor="weeksToTarget" className="text-sm font-medium text-gray-300">Weeks to Target</Label>
                        <Input id="weeksToTarget" type="number" value={inputs.weeksToTarget || ''} onChange={(e) => handleInputChange('weeksToTarget', parseInt(e.target.value) || undefined)} min="1" className="mt-1 bg-zinc-800 border-zinc-700" />
                    </div>
                </div>
              )}


              <Button onClick={calculateTdee} disabled={isCalculating} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-bold py-3.5 text-lg mt-6 h-14">
                {isCalculating ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Flame size={20} className="mr-2.5" />}
                ...
                <Card className="bg-zinc-900/80 border-zinc-700/70 shadow-2xl text-center backdrop-blur-sm rounded-xl">
                  <CardHeader className="pb-4 pt-6">
                    <CardTitle className="text-xl md:text-2xl text-primary flex items-center justify-center gap-2">
                      <TargetIcon size={24}/>Your Estimated Needs
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-5 pb-6 px-5">
                    ...
                    {result.tdee && (
                       <div className="py-3 border-t border-b border-zinc-700/50">
                        ...
                        <p className="text-4xl font-extrabold text-primary">{result.tdee} kcal/day</p>
                        <p className="text-xs text-gray-500">({inputs.activityLevel ? inputs.activityLevel.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()) : 'Activity not specified'})</p>
                      </div>
                    )}
                    {result.goalCalories && (
                      <div className="py-3">
                        <p className="text-sm text-gray-400 mb-1">Target Daily Calories for <span className='font-semibold text-gray-300'>{inputs.goal.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</span></p>
                    ...
                    {result.protein && result.fat && result.carbs && result.goalCalories && (
                        <div className="pt-4 space-y-3 text-sm text-left">
                            ...
                            <div className="grid grid-cols-3 gap-3 text-center">
                                <div>
                                    <p className="font-medium text-primary">Protein</p>
                                    <p className="text-gray-200">{result.protein.min} - {result.protein.max}g</p>
                                </div>
                                <div>
                                    <p className="font-medium text-primary">Fat</p>
                                    <p className="text-gray-200">{result.fat.min} - {result.fat.max}g</p>
                                </div>
                                <div>
                                    <p className="font-medium text-primary">Carbs</p>
                    ...
                    <div className="text-xs text-gray-500 flex items-start gap-2 p-3 bg-black/20 rounded-md border border-zinc-700/40">
                        ...
                        <span>
                        This tool provides estimates for informational purposes. It does not qualify as medical or nutritional advice. Consult with a healthcare professional or registered dietitian before making significant changes to your diet or exercise plan. True fitness optimization considers many individual factors.
                        </span>
                    </div>
                  </CardContent>
                </Card>
