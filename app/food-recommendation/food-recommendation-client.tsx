...
import { Apple, Target, Zap, Users, Sparkles, Loader2, CheckCircle, Heart, Dumbbell, Timer, Star, TrendingUp } from "lucide-react";
...
const fitnessGoals: FitnessGoal[] = [
  { id: "bulking", label: "Muscle Building (Bulking)", icon: Dumbbell, color: "text-blue-400" },
  { id: "cutting", label: "Fat Loss (Cutting)", icon: TrendingUp, color: "text-red-400" },
  { id: "maintenance", label: "Maintain Weight", icon: Target, color: "text-green-400" },
  { id: "strength", label: "Strength Training", icon: Zap, color: "text-purple-400" },
];
...
const activityLevels: ActivityLevel[] = [
  ...
  { id: "moderate", label: "Moderate Activity (3-5 days/week)", multiplier: 1.55 },
...
export default function FoodRecommendationClient() {
  ...
  useEffect(() => {
    ...
    if (storedProfile) {
      try {
        ...
        if (profile.name && (profile.gender === "male" || profile.gender === "female") && profile.heightCm > 0 && profile.currentWeightKg > 0) {
  ...
  const calculateCalories = (profile: UserProfile, activityLevel: string, goal: string): number => {
    const activityMultiplier = activityLevels.find((a) => a.id === activityLevel)?.multiplier ?? 1.55;

    // Basic BMR calculation (Mifflin-St Jeor)
    const bmr =
      profile.gender === "male"
        ? 88.362 + 13.397 * profile.currentWeightKg + 4.799 * profile.heightCm - 5.677 * 25 // assuming age 25
        : 447.593 + 9.247 * profile.currentWeightKg + 3.098 * profile.heightCm - 4.330 * 25;
  ...
  const generateRecommendations = async () => {
    ...
    if (!userProfile) {
      toast({
        ...
        description: "Please set up your user profile to get personalized recommendations.",
      });
      return;
    }
    ...
    try {
      ...
      const calories = calculateCalories(userProfile, selectedActivity, selectedGoal);
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        >
          <header className="text-center mb-10 md:mb-12">
            <Apple size={52} className="mx-auto mb-5 text-primary" />
            <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">
              GYM BRO <span className="text-primary">Food Recommendations</span>
            </h1>
            <p className="text-gray-400 mt-4 text-md md:text-lg max-w-2xl mx-auto">
              Get personalized food recommendations based on your fitness goals and activity level. Fuel your journey to greatness!
            </p>
          </header>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <span className="text-gray-300">
                      Profile: <span className="text-white font-semibold">{userProfile.name}</span> | Height:{" "}
                      <span className="text-white">{userProfile.heightCm}cm</span> | Weight:{" "}
                      <span className="text-white">{userProfile.currentWeightKg}kg</span>
                    </span>
                  </div>
                </CardContent>
              </Card>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
              <Card className="bg-zinc-900/70 border-zinc-700/60 shadow-xl backdrop-blur-sm rounded-xl">
                <CardContent className="p-6 text-center">
                  <p className="text-gray-400">No user profile found. Please set up your profile to get personalized recommendations.</p>
                  <Button variant="link" className="mt-2 text-primary" onClick={() => toast({
                    title: "Profile Setup",
                    description: "Profile setup is not implemented yet. Please add your details manually.",
                  })}>
                    Set up profile
                  </Button>
                </CardContent>
          ...
          <Card className="bg-zinc-900/70 border-zinc-700/60 shadow-2xl backdrop-blur-sm rounded-xl mb-8">
            <CardHeader>
              ...
              <CardDescription className="text-gray-400">
                Choose your primary fitness objective to get tailored food recommendations.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-zinc-900/70 border-zinc-700/60 shadow-2xl backdrop-blur-sm rounded-xl mb-8">
            <CardHeader>
              <CardTitle className="text-2xl text-white flex items-center">
                <Zap size={24} className="mr-3 text-primary" />
                Activity Level
              </CardTitle>
              <CardDescription className="text-gray-400">
              ...
              <div className="space-y-3">
                {activityLevels.map((level) => (
                  <motion.div key={level.id} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}>
                    <Button
                      variant={selectedActivity === level.id ? "default" : "outline"}
                      className={cn(
                        "w-full h-auto p-4 justify-start text-left",
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        ...
                        <span className="text-sm opacity-70">×{level.multiplier}</span>
                      </div>
                    </Button>
                  </motion.div>
                ))}
          ...
          <div className="text-center mb-8">
            <Button
              ...
              className="bg-primary text-primary-foreground hover:bg-primary/90 font-bold py-4 px-8 text-lg h-auto"
              size="lg"
            >
              {isLoading ? (
                <>
          ...
          {dailyCalories > 0 && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
              <Card className="bg-gradient-to-r from-primary/20 to-primary/10 border-primary/30 shadow-xl">
                <CardContent className="p-6 text-center">
                  <Heart size={24} className="mx-auto mb-2 text-primary" />
                  <h3 className="text-xl font-bold text-white mb-1">Your Daily Calorie Target</h3>
                  <p className="text-3xl font-extrabold text-primary">{dailyCalories} calories</p>
                  <p className="text-sm text-gray-300 mt-2">Based on your profile, goal, and activity level</p>
                </CardContent>
              </Card>
            </motion.div>
          )}
          ...
          <AnimatePresence mode="wait">
            {isLoading && (
              ...
              >
                ...
                <p className="text-xl font-semibold text-white">GYM BRO AI is crafting your meal plan...</p>
            ...
            {recommendations.length > 0 && !isLoading && (
              ...
              >
                <h2 className="text-2xl font-bold text-white mb-6 text-center">Your Personalized Food Recommendations</h2>
                      ...
                      <Card className="bg-zinc-900/80 border-zinc-700/70 hover:border-zinc-600/70 shadow-xl backdrop-blur-sm rounded-xl h-full transition-all duration-300 hover:shadow-2xl hover:scale-[1.02]">
                        <CardHeader className="pb-3">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <CardTitle className="text-lg text-white mb-1">{food.name}</CardTitle>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="text-center py-2 bg-zinc-800/50 rounded-lg">
                            ...
                            <p className="text-xs text-gray-400">calories per serving</p>
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-2 text-center text-sm">
                            <div>
                              ...
                              <p className="font-semibold text-white">{food.protein}</p>
                            </div>
                            <div>
                              <p className="text-gray-400">Carbs</p>
                              <p className="font-semibold text-white">{food.carbs}</p>
                            </div>
                            <div>
                              <p className="text-gray-400">Fat</p>
                              <p className="font-semibold text-white">{food.fat}</p>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-white">{food.bestTime}</span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-gray-400">Prep Time:</span>
                              <div className="flex items-center gap-1">
                                <Timer size={12} className="text-gray-400" />
                                <span className="text-white">{food.prepTime}</span>
                              </div>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-gray-400">Difficulty:</span>
                              <span className={cn("font-medium", getDifficultyColor(food.difficulty))}>
                                {food.difficulty}
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-gray-400">Serving:</span>
                              <span className="text-white text-xs">{food.servingSize}</span>
                            </div>
                          </div>
                  ...
                  <Card className="bg-zinc-900/60 border-zinc-700/60 inline-block">
                    <CardContent className="p-4">
                      <p className="text-sm text-gray-400 flex items-center gap-2">
                        ...
                        These recommendations are personalized based on your fitness goals and activity level. Adjust
                        portions according to your specific calorie needs.
                      </p>
                    </CardContent>
                  </Card>
                </div>
